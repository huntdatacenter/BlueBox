...
options:
  ...
  version:
    ...
    required: false
  state:
    description: State in which to leave the Python package. "present"
                 will install a package of the specified version
                 if it is not installed (will not upgrade to latest if
                 version is unspecified - will only install latest);
                 "latest" will both install and subsequently upgrade
                 a package to the latest version on each run;
                 "absent" will uninstall the package if installed.
    required: false
    default: present
...
def run_package_operation(
    conda, name, version, state, dry_run,
    command_runner, on_failure, on_success
):
    """
    Runs Conda package operation.

    This method is intentionally decoupled from `AnsibleModule`
    to allow it to be easily tested in isolation.
    ...
    :param command_runner: method that executes a given Conda command
    (given as list of string arguments), which returns
    JSON and returns a tuple where the first argument is the outputted
    JSON and the second is anything written to stderr
    ...
    """
    correct_version_installed = check_package_installed(
        command_runner, conda, name, version)

    # TODO: State should be an "enum" (or whatever the Py2.7 equivalent is)
    if not correct_version_installed and state != 'absent':
        try:
            output, stderr = install_package(
                command_runner, conda, name, version, dry_run=dry_run)
            on_success(changed=True, output=output, stderr=stderr)
        except CondaPackageNotFoundError:
            on_failure(msg='Conda package "{}" not found'.format(
                get_install_target(name, version)
            ))

    elif state == 'absent':
        try:
            output, stderr = uninstall_package(
                command_runner, conda, name, dry_run=dry_run)
            on_success(changed=True, output=output, stderr=stderr)
        except CondaPackageNotFoundError:
...
def check_package_installed(command_runner, conda, name, version):
    ...
    :param version: the version of the package to check if installed
                    (`None` if check for latest)
    :return: `True` if a package with the given name and version is installed
    :raises CondaUnexpectedOutputError: if the JSON returned by Conda
                                        was unexpected
    """
    cmd = [conda, 'install', '--json', '--dry-run',
           get_install_target(name, version)]
    output, stderr = run_conda_package_command(
        command_runner, name, version, cmd)

    inst_msg = 'All requested packages already installed.'
    if output.get('message', '') == inst_msg:
        return True
...
def install_package(command_runner, conda, name, version=None, dry_run=False):
    """
    Install a package with the given name and version.
    Version will default to latest if `None`.
    """
    command = [conda, 'install', '--yes', '--json',
               get_install_target(name, version)]
    if dry_run:
...
def run_conda_package_command(command_runner, name, version, command):
    ...
    :raises CondaPackageNotFoundError: if the package referred to
                                       by this command is not found
    """
    try:
        return command_runner(command)
    except CondaCommandJsonDescribedError as e:
        if e.output.get('exception_name', '') in (
...
def get_install_target(name, version):
    ...
    if version is not None:
        install_target = '{}={}'.format(name, version)
...
class CondaCommandError(Exception):
    ...
    def __init__(self, command, stdout, stderr):
        ...
        stdout = ' stdout: {}.'.format(
            self.stdout if self.stdout.strip() else ''
        )
...
class CondaCommandJsonDescribedError(CondaCommandError):
    ...
    def __init__(self, command, output, stderr):
        ...
        super(CondaCommandJsonDescribedError, self).__init__(
            command, json.dumps(output), stderr)
...
class CondaPackageNotFoundError(Exception):
    ...
    def __int__(self, name, version):
        ...
        super(CondaPackageNotFoundError, self).__init__(
            'Conda package "{}" not found'.format(
                get_install_target(self.name, self.version)
            ))
...
class CondaUnexpectedOutputError(Exception):
    ...
    def __int__(self, output, stderr):
        ...
        stderr = 'stderr: {}' % self.stderr if self.stderr.strip() else ''
        super(CondaUnexpectedOutputError, self).__init__(
            'Unexpected output from Conda'
            '(may be due to a change in Conda\'s output format): '
            '"{}".{}'.format(self.output, stderr))
...
def _main():
    ...
    if state == 'latest' and version is not None:
        module.fail_json(
            msg='`version` must not be set if `state == "latest"`'
                ' (`latest` upgrades to newest version)'
        )
    ...
    run_package_operation(
        conda, name, version, state, module.check_mode,
        command_runner, module.fail_json, module.exit_json
    )
